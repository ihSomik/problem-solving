  ...
  for (int i = 0; i < r; i++) {
    for (int j = 0; j < c; j++) {
      if (j + 3 < c) {
        ...
        maxProduct = max(maxProduct, horizontalProduct);
      }

      if (i + 3 < r) {
        int verticalProduct =
            grid[i][j] * grid[i + 1][j] * grid[i + 2][j] * grid[i + 3][j];
        maxProduct = max(maxProduct, verticalProduct);
      }

      if (i + 3 < r && j + 3 < c) {
        int diagonalProduct1 = grid[i][j] * grid[i + 1][j + 1] *
                               grid[i + 2][j + 2] * grid[i + 3][j + 3];
        maxProduct = max(maxProduct, diagonalProduct1);
      }

      if (i + 3 < r && j - 3 >= 0) {
        int diagonalProduct2 = grid[i][j] * grid[i + 1][j - 1] *
                               grid[i + 2][j - 2] * grid[i + 3][j - 3];
        maxProduct = max(maxProduct, diagonalProduct2);
      }
    }
  }
