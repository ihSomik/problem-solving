...
long minTime(vector<int> files, int numCores, int limit) {
    ...
    for (int file : files)
        if (file % numCores == 0)
            divisible.push_back(file);
        else
            nonDivisible.push_back(file);

    sort(divisible.rbegin(), divisible.rend());

    long long time(0);

    limit = min(limit, (int)divisible.size());

    for (int i = 0; i < limit; i++) time += divisible[i] / numCores;

    for (int i = limit; i < divisible.size(); ++i) time += divisible[i];

    for (int t : nonDivisible) time += t;
